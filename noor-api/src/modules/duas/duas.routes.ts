// ============================================================
// NOOR API — Duas & Adhkar Module Routes
// ============================================================
 ...
 */
duasRouter.get('/categories', (_req: Request, res: Response) => {
  res.json({
    success: true,
    data: DUA_CATEGORIES
  });
 ...
 */
duasRouter.get('/', (req: Request, res: Response) => {
  const cat = req.query.category as string;
  let items = DUAS_LIST;
  if (cat && cat !== 'all') {
    items = DUAS_LIST.filter(d => d.category === cat);
  }
  res.json({
    success: true,
    data: items,
 ...
 * GET /api/v1/duas/:id
 */
duasRouter.get('/:id', (req: Request, res: Response) => {
  const dua = DUAS_LIST.find(d => d.id === req.params.id);
  if (!dua) {
    res.status(404).json({ success: false, error: 'Dua not found' });
    return;
  }
  res.json({
    success: true,
